...
//    private static Set<Record> ERRORS = new LinkedHashSet<Record>();
    private static Set<Record> EXCEPTIONS = new LinkedHashSet<Record>();
...
//            ERRORS.add(r);
    ...
    public static synchronized void catchThrowable(Object _this) {
        if (TABLE.isEmpty() || _this == null) return;

        if (_this.getClass().getName().endsWith("ClassNotFoundException")) return;

        StackTraceElement[] trace = ((Throwable)_this).getStackTrace();
        for (StackTraceElement el : trace) {
            if (el.getClassName().equals("org.exist.storage.BrokerPool") && el.getMethodName().equals("<init>")) {
                //ignore exception during initialization?
                return;
            }
            if (el.getClassName().startsWith("org.exist.agent.")) {
                //ignore because our call
                return;
            }
        }

        for (Record r : TABLE.values()) {
            if (r.thread == Thread.currentThread()) {

                r.exception((Throwable)_this);
                EXCEPTIONS.add(r);
                break;
            }
        }
    ...
    static synchronized void dumpExceptions(Writer w) {
        PrintWriter pw = new PrintWriter(w);
        Record[] records = EXCEPTIONS.toArray(new Record[0]);
        pw.println(records.length+" exceptions");
        int i=0;
    ...
    static void printout(Throwable ex, PrintWriter pw) {
        ex.printStackTrace(pw);
        //printout(ex.getStackTrace(), pw);
    }

    static void printout(StackTraceElement[] trace, PrintWriter pw) {
        int i = 0;
